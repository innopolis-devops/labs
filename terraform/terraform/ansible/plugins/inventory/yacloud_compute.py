...
DOCUMENTATION = '''
    ...
    options:
        ...
        yacloud_token_storage:
            description: The type of yacloud Oauth token storage. One of following <raw>, <file>, <env_var>
            required: True
            choices: ['raw', 'file', 'env_var']
        yacloud_token:
            description: If yacloud_token_storage is <raw>, then raw Oauth token, if <file>, then file name, if <env_var>, then name of environment variable
            required: True
        yacloud_user:
            description: Default username used in hosts
            required: False
        yacloud_clouds:
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _init_client(self):
        if self.get_option('yacloud_token_storage') == 'raw':
            _token = str(self.get_option('yacloud_token'))
        elif self.get_option('yacloud_token_storage') == 'file':
            with open(self.get_option('yacloud_token'), 'r') as file:
                _token = str(file.read().strip())
        else:
            _token = os.getenv(self.get_option('yacloud_token'))
        
        sdk = yandexcloud.SDK(token=_token)
        ...
        for instance in self.hosts:
            ...
            if instance["status"] == "RUNNING":
                ...
                if ip:
                    ...
                    self.inventory.set_variable(instance["name"], 'ansible_user', self.get_option('yacloud_user'))
